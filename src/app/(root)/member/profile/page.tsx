...
export default function ProfilePage() {
  ...
  async function fetchMemberData(): Promise<MemberType[]> {
    ...
    const fitnessGoalResponse = await fetch(`/fitnessGoal?username=${username}`);
      ...
      <section className="flex flex-wrap w-full gap-10">
        <div className="w-[30%] rounded-lg border-[1px] p-5 flex flex-col gap-y-3 relative">
          <span className="flex w-full lg:items-center justify-between lg:flex-row flex-col items-start gap-y-3 ">
            ...
            <button className="border-2 text-sm shadow-sm  px-4 py-2 rounded-md hover:bg-slate-400/40">Transactions</button>
          </span>
          <h4 className="absolute bottom-5 right-5 md:text-6xl font-bold text-3xl ">${fees !== undefined ? `${fees}` : <Spinner />}</h4>
        </div>
